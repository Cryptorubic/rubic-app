...
export class ContractsListComponent implements OnInit {
  ...
  constructor(
    ...
    contractListData.map((item) => {
      const leftTime =
        (new Date(item.stop_date).getTime() -
          (new Date().getTime() - serverDateTimeRange)) /
        1000;
  ...
  public deleteContractConfirm() {
    ...
    if (this.contractForDeleting.contract_type === 20) {
      this.contractsService
        .deleteContract(this.contractForDeleting)
        .then(() => {});
    } else {
      this.contractsService
        .deleteSwap(this.contractForDeleting.id)
        .then(() => {});
    }
  ...
  public applySort(sortName?: any) {
    ...
    switch (this.selectedFilter.name) {
      ...
      case 'expire':
        this.contractsList = this.contractsList.sort((contract1, contract2) => {
          const date1 =
            contract1.stop_date || contract1.contract_details.stop_date;
          const date2 =
            contract2.stop_date || contract2.contract_details.stop_date;
        ...
        this.contractsList = this.contractsList.sort((contract1, contract2) => {
          if (this.selectedFilter.asc) {
            return this.states[contract1.state].NUMBER >
              this.states[contract2.state].NUMBER
              ? 1
              : -1;
          } else {
            return this.states[contract2.state].NUMBER >
              this.states[contract1.state].NUMBER
              ? 1
              : -1;
          }
        });
      ...
      default:
        this.contractsList = this.contractsList.sort((contract1, contract2) => {
          return new Date(contract2.created_date) <
            new Date(contract1.created_date)
            ? -1
            : 1;
        });
    }
