...
export class ContractsService {
  ...
  public getPastTrades(filters?) {
    return this.httpService
      .get(`get_non_active_swap3/`, filters)
      .toPromise()
  ...
  public getPublicContractsList() {
    ...
    return new Promise((resolve, reject) => {
      const resolveList = () => {
        if (allList.trades && allList.contracts) {
          ...
          if (expiredTrades.contracts.length || expiredTrades.trades.length) {
            this.httpService
              .post('set_swap3_expired/', expiredTrades)
              .toPromise()
              .then(res => {
                return res;
              });
      ...
      this.httpService
        .get('get_public_contracts/')
        .toPromise()
        .then(result => {
          allList.contracts = result.filter(contract => {
            const noExpired =
              new Date(contract.contract_details.stop_date).getTime() > new Date().getTime();
            if (!noExpired) {
  ...
  public getContractByPublic(publicLink) {
    return this.httpService
      .get(`get_contract_for_unique_link/`, {
