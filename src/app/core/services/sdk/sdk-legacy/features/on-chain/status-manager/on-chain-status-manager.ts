...
export class OnChainStatusManager {
  ...
  public async getBridgersSwapStatus(srcTxHash: string, slippage?: number): Promise<TxStatusData> {
    ...
    const srcTxStatus = await tronAdapter.getSrcTxStatus(BLOCKCHAIN_NAME.TRON, srcTxHash);
  ...
  private async getSrcStatusRecursive(
    ...
    const srcTxStatus = await tronAdapter.getSrcTxStatus(blockchain, srcTxHash);
  ...
  public async getStellarSwapStatus(srcTxHash: string): Promise<TxStatusData> {
    try {
